...
const Contact = () => {
    return (
        <section className="w-full pb-32 rounded-t-3xl bg-[var(--color-accent)] px-10 pt-10">
            ...
            >
                Get In <span className="text-[var(--color-black)] italic">Touch</span>
            </motion.h2>
                <motion.svg
                    className="absolute -bottom-2 left-0 w-full h-4 text-[var(--color-white)]"
                    viewBox="0 0 200 12"
...
const ContactLink = ({ icon, label, value, link }) => (
    ...
    >
        <div className="w-12 h-12 rounded-full border border-[var(--color-divider)] flex items-center justify-center text-[var(--color-white)] group-hover:bg-[var(--color-black)] group-hover:text-white transition-all duration-300">
            {icon}
        </div>
        <div>
            <p className="text-sm text-[var(--color-text-muted)] font-sans mb-1 uppercase tracking-wider">{label}</p>
            <p className="text-lg font-serif text-white group-hover:text-[var(--color-black)] transition-colors">{value}</p>
        </div>
    </a>
);
